...
{
	for (auto& a : dice_set[current]->get_sides())
	{
		if (current == dice_set.size() - 1)
		{
			current_combination.push_back(a->get_side_index());
			result.push_back({ current_combination, cur_chance * dice_set[current]->get_chance() });
			current_combination.pop_back();
		}
		else
		{
			current_combination.push_back(a->get_side_index());
			calculate_comb(result, current + 1, cur_chance * dice_set[current]->get_chance(), current_combination);
			current_combination.pop_back();
		}
	}
}
